//PageFrame - base64 Lib v1.0 © copyright by TitanNano / Jovan Gerodetti - titannano.de
...
window.lib['b64']= {
		decode : function(string){
			...
			for(var i= 0; i < string.length; i+= 4){
				...
				var x= (string[i+1] === '+') ? '\\+' : string[i+1];
				var oB2= catalog.search(x);
				var x= (string[i+2] === '+') ? '\\+' : string[i+2];
				var oB3= catalog.search(x);
				var x= (string[i+3] === '+') ? '\\+' : string[i+3];
